          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd v-for="sale in saleAttr.spuSaleAttrValueList" 
                :key="sale.id" :class="{active:sale.isChecked==1}"
                @click="changeActive(sale,saleAttr.spuSaleAttrValueList)">
            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a href="javascript:" class="mins" @click="skuNum>1?skuNum--:0">-</a>
              </div>
              <div class="add">
  ...
  export default {
    ...
    methods:{
      ...
      async addShopcar(){
        ...
        /* 当前这里是派发一个action，向服务器发请求，判断加入购物车是成功还是失败，进行相应的操作
        this.$store.dispatch('detail/addOrUpdateShopcar',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        上面代码说明：调用仓库中的addOrUpdateShopcar，这个方法加上asyc，返回一定是一个promise
        要么成功|要么失败 */
        try {
          await this.$store.dispatch('detail/addOrUpdateShopcart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
          //路由跳转
          //在路由跳转的时候将产品的信息带给下一级路由组件
          //一些简单的数据skuNum，通过query形式给路由组件传递罐区
          // 产品信息的数据【比较复杂：skuinfo】，通过会话存储（不持久化，会话结束数据在消失）
          // 本地存储|回话存储，一般存储的是字符串

          console.log("sdfjeifjejji")
